...
export const TEMPDB_IO_BASELINES: TempDbIoBaseline[] = [
  {
    ...
    criticalText: { en: '5+: classic metadata contention.', es: '5+: contencion clasica de metadatos.' },
...
WHERE wait_type LIKE 'PAGELATCH%'
  ...
  },
  {
    ...
    whatToRead: {
      ...
      es: 'Util cuando los spills pisan disco y empieza a salir PAGEIOLATCH_SH.',
    },
    unit: 'ms',
    min: 0,
    max: 40,
    ...
    warningText: { en: '8-20 ms: watch bursts.', es: '8-20 ms: vigila las rafagas.' },
...
ORDER BY avg_read_ms DESC;`,
  },
  {
    ...
    whatToRead: {
      ...
      es: 'Aqui es donde PAGEIOLATCH_SH suele empezar a tener sentido.',
    },
    unit: 'ms',
    min: 0,
    max: 50,
    ...
    warningText: { en: '8-20 ms: acceptable only in short bursts.', es: '8-20 ms: aceptable solo en rafagas cortas.' },
...
ORDER BY avg_read_ms DESC;`,
  },
  {
    ...
    whatToRead: {
      ...
      es: 'Los checkpoints o rafagas de escritura fuertes se ven aqui.',
    },
    unit: 'ms',
    min: 0,
    max: 40,
    ...
    warningText: { en: '5-15 ms: watch checkpoint bursts.', es: '5-15 ms: vigila rafagas de checkpoint.' },
...
ORDER BY avg_write_ms DESC;`,
  },
  {
    ...
    warningText: { en: '3-8 ms: watch VLF and growth pattern.', es: '3-8 ms: vigila VLF y el patron de growth.' },
...
ORDER BY avg_write_ms DESC;`,
  },
  {
    ...
    whatToRead: {
      ...
      es: 'Picos cortos estan bien. Una cola sostenida no.',
    },
    unit: 'peticiones',
    min: 0,
    max: 20,
    ...
    warningText: { en: '3-8: watch together with latency.', es: '3-8: miralo junto con la latencia.' },
    criticalText: { en: '9+: storage is backing up.', es: '9+: el almacenamiento se esta atascando.' },
...
ORDER BY io_pending_ms_ticks DESC;`,
  },
  {
    ...
    summary: {
      ...
      es: 'Con que frecuencia Lazy Writer vacia paginas para recuperar buffers.',
    },
    whatToRead: {
      en: 'Persistent movement means memory pressure, not just background churn.',
      es: 'Movimiento persistente significa presion de memoria, no solo ruido de fondo.',
    },
    unit: '/sec',
    min: 0,
    max: 60,
    ...
    warningText: { en: '6-20/sec: memory pressure may be building.', es: '6-20/sec: puede crecer presion de memoria.' },
...
ORDER BY counter_name;`,
  },
  {
    ...
    summary: {
      ...
      es: 'Cuantas veces un worker no consigue un buffer libre al instante.',
    },
    whatToRead: {
      en: 'This is one of the clearest memory-pain counters.',
      es: 'Es uno de los contadores mas claros de dolor de memoria.',
    },
    unit: '/sec',
    min: 0,
    max: 12,
...
export const TEMPDB_IO_DIAGNOSTIC_CASES: TempDbIoDiagnosticCase[] = [
  {
    ...
    question: {
      ...
      es: 'Aparecen muchas #temp y todo se frena. Que sospechas primero?',
    },
    summary: {
      en: 'Classic contention on allocation pages, not on storage latency.',
      es: 'Contencion clasica en paginas de asignacion, no en latencia de disco.',
    },
    suspicion: {
    ...
    choices: [
      { id: 'disk', label: { en: 'Slow storage', es: 'Disco lento' }, explanation: { en: 'Latency is fine. This is not the first suspect.', es: 'La latencia esta bien. No es la primera sospecha.' } },
      { id: 'metadata', label: { en: 'Allocation contention', es: 'Contencion de asignacion' }, explanation: { en: 'Correct: PFS/GAM/SGAM are the bottleneck.', es: 'Correcto: PFS/GAM/SGAM son el cuello.' } },
    ...
    dbaFocus: { en: 'Prove the hot pages before changing TempDB.', es: 'Demuestra las paginas calientes antes de cambiar TempDB.' },
    dbaActions: [
      { en: 'Capture waits on 2:1:1/2/2/3.', es: 'Captura waits en 2:1:1/2/2/3.' },
      { en: 'Review file count, size and growth.', es: 'Revisa numero, tamaño y growth.' },
      { en: 'Then chase spill-heavy queries if needed.', es: 'Despues persigue queries con spills si hace falta.' },
    ],
    caution: { en: 'More files only help if size and growth are balanced.', es: 'Mas archivos solo ayudan si tamaño y growth estan equilibrados.' },
...
FROM tempdb.sys.database_files;`,
    followUp: [
      { en: 'If waits fall after file fan-out, the diagnosis was right.', es: 'Si los waits caen tras repartir archivos, el diagnostico era correcto.' },
  ...
  },
  {
    ...
    question: {
      ...
      es: 'La query se frena, TempDB crece y el plan avisa de spills. Cual es la ruta probable?',
    },
    summary: {
    ...
    suspicion: {
      ...
      es: 'El DBA sospecha de grant o estimacion, no de I/O bruto.',
    },
    ...
    choices: [
      ...
      { id: 'log', label: { en: 'Log bottleneck', es: 'Cuello del log' }, explanation: { en: 'WRITELOG would tell a different story.', es: 'WRITELOG contaria otra historia.' } },
    ...
    dbaActions: [
      ...
      { en: 'Then validate stats and parameter sensitivity.', es: 'Despues valida estadisticas y sensibilidad a parametros.' },
    ],
    caution: { en: 'More RAM can hide the symptom and leave the estimate problem alive.', es: 'Mas RAM puede tapar el sintoma y dejar vivo el problema de estimacion.' },
...
-- Re-run and inspect the actual execution plan for spill warnings.`,
    followUp: [
      { en: 'If estimates are wrong, fix stats or predicates first.', es: 'Si las estimaciones fallan, corrige estadisticas o predicados primero.' },
  ...
  },
  {
    ...
    question: {
      ...
      es: 'La ejecucion 1 va lenta y la 2 rapida. Que mira primero el DBA?',
    },
    summary: {
      en: 'This smells like cold cache before it smells like blocking.',
      es: 'Esto huele antes a cache fria que a bloqueo.',
    },
    suspicion: {
      en: 'The DBA wants to separate cold cache from a bad scan.',
      es: 'El DBA quiere separar cache fria de un mal scan.',
    },
    ...
    observations: [
      ...
      { label: { en: 'Physical reads', es: 'Lecturas fisicas' }, value: '480,000', tone: 'amber' },
      { label: { en: 'Read latency', es: 'Latencia de lectura' }, value: '7 ms', tone: 'emerald' },
    ],
    choices: [
      { id: 'cache', label: { en: 'Cold cache', es: 'Cache fria' }, explanation: { en: 'Correct: pages are warm on the second run.', es: 'Correcto: las paginas quedan calientes en la segunda pasada.' } },
      { id: 'blocking', label: { en: 'Blocking chain', es: 'Cadena de bloqueos' }, explanation: { en: 'Blocking would not vanish because cache got warm.', es: 'El bloqueo no desaparece porque la cache se caliente.' } },
      { id: 'tempdb', label: { en: 'TempDB issue', es: 'Problema de TempDB' }, explanation: { en: 'This pattern is base-page I/O, not TempDB metadata.', es: 'Este patron es I/O de paginas base, no metadatos de TempDB.' } },
    ],
    ...
    dbaFocus: { en: 'Measure physical versus logical reads.', es: 'Mide lecturas fisicas frente a logicas.' },
    dbaActions: [
      { en: 'Run with SET STATISTICS IO, TIME ON.', es: 'Ejecuta con SET STATISTICS IO, TIME ON.' },
      { en: 'Check PAGEIOLATCH waits during the first run only.', es: 'Mira PAGEIOLATCH solo durante la primera ejecucion.' },
      { en: 'If logical reads are huge, review the plan next.', es: 'Si las lecturas logicas son enormes, revisa el plan despues.' },
    ],
    caution: { en: 'Cold cache is a symptom. It can still hide a bad scan.', es: 'La cache fria es un sintoma. Aun asi puede esconder un mal scan.' },
...
WHERE wait_type LIKE 'PAGEIOLATCH%';`,
    followUp: [
      { en: 'If logical reads stay massive, fix plan or index shape.', es: 'Si las lecturas logicas siguen masivas, corrige la forma del plan o del indice.' },
      { en: 'If only physical reads are high at first, the cache story is enough.', es: 'Si solo son altas las fisicas al principio, la historia de cache basta.' },
    ],
  },
  {
    id: 'writelog',
    ...
    question: {
      ...
      es: 'Los commits cortos se sienten pegajosos y WRITELOG no deja de salir. Cual es la sospecha fuerte?',
    },
    summary: {
      en: 'The pain is on the log path, not on data reads.',
      es: 'El dolor esta en la ruta del log, no en las lecturas de datos.',
    },
    suspicion: {
    ...
    choices: [
      { id: 'log', label: { en: 'Log path / VLF issue', es: 'Ruta de log / VLF' }, explanation: { en: 'Correct: WRITELOG plus high log latency points here.', es: 'Correcto: WRITELOG mas alta latencia de log apunta aqui.' } },
      { id: 'reads', label: { en: 'Random reads', es: 'Lecturas aleatorias' }, explanation: { en: 'That would show a different wait and latency pattern.', es: 'Eso mostraria otro patron de waits y latencia.' } },
      { id: 'cpu', label: { en: 'CPU pressure', es: 'Presion de CPU' }, explanation: { en: 'CPU pressure would show runnable queues, not this.', es: 'La presion de CPU mostraria colas runnable, no esto.' } },
    ],
    correctChoiceId: 'log',
    diagnosis: {
...
FROM sys.dm_db_log_info(DB_ID());`,
    followUp: [
      ...
      { en: 'If latency is fine, inspect batching and sync replica hardening.', es: 'Si la latencia esta bien, inspecciona batching y endurecimiento en replica sincrona.' },
    ],
  },
  {
    id: 'version-store',
    ...
    question: {
      ...
      es: 'TempDB no para de crecer bajo RCSI o Snapshot. Cual es la causa probable?',
    },
    summary: {
      en: 'Version store is pinned by long readers.',
      es: 'El version store esta inmovilizado por lectores largos.',
    },
    suspicion: {
      en: 'The DBA now suspects old snapshot readers more than latch contention.',
      es: 'El DBA sospecha ahora mas de lectores snapshot antiguos que de contencion de latches.',
    },
    badges: ['version store', 'RCSI', 'snapshot'],
    accent: 'cyan',
    observations: [
      { label: { en: 'Version store', es: 'Version store' }, value: '14 GB', tone: 'rose' },
      { label: { en: 'Oldest snapshot', es: 'Snapshot mas antigua' }, value: '47 min', tone: 'rose' },
    ...
    choices: [
      { id: 'reader', label: { en: 'Long snapshot reader', es: 'Lector snapshot largo' }, explanation: { en: 'Correct: cleanup cannot reclaim versions yet.', es: 'Correcto: la limpieza todavia no puede recuperar versiones.' } },
    ...
    diagnosis: {
      ...
      es: 'Encuentra el lector mas antiguo antes de limitarte a dar mas espacio a TempDB.',
    },
    dbaFocus: { en: 'Identify the oldest snapshot transaction.', es: 'Identifica la transaccion snapshot mas antigua.' },
    dbaActions: [
      ...
      { en: 'Fix the reader pattern before growing TempDB.', es: 'Corrige el patron del lector antes de hacer crecer TempDB.' },
    ],
...
ORDER BY elapsed_time_seconds DESC;`,
    followUp: [
      { en: 'If one report holds the oldest snapshot, fix that workload first.', es: 'Si un informe mantiene la snapshot mas antigua, corrige esa carga primero.' },
      { en: 'If versions are low but waits are high, go back to metadata contention.', es: 'Si las versiones son bajas pero los waits altos, vuelve a la contencion de metadatos.' },
    ],
  },
  {
    id: 'memory',
    ...
    question: {
      ...
      es: 'Las lecturas se frenan pero la latencia de disco es decente. Que señal importa mas ahora?',
    },
    summary: {
    ...
    suspicion: {
      ...
      es: 'El DBA sospecha de grants o de memoria maxima antes de culpar a los discos.',
    },
    ...
    choices: [
      { id: 'memory', label: { en: 'Memory pressure', es: 'Presion de memoria' }, explanation: { en: 'Correct: the box is fighting for free buffers.', es: 'Correcto: la maquina pelea por buffers libres.' } },
    ...
    dbaActions: [
      ...
      { en: 'Validate SQL memory versus host pressure.', es: 'Valida la memoria de SQL frente a la presion del host.' },
    ],
    caution: { en: 'A low PLE alone is not enough. The pattern matters.', es: 'Un PLE bajo por si solo no basta. Importa el patron.' },
    query: `SELECT counter_name, cntr_value
FROM sys.dm_os_performance_counters
...
ORDER BY requested_memory_kb DESC;`,
    followUp: [
      { en: 'If grants are pending, chase the spill story next.', es: 'Si hay grants pendientes, persigue despues la historia del spill.' },
      { en: 'If grants are calm, inspect clerks or host pressure.', es: 'Si las grants estan tranquilas, inspecciona clerks o presion del host.' },
    ],
  },
  {
    id: 'checkpoint',
    title: { en: 'Periodic write bursts', es: 'Rafagas periodicas de escritura' },
    question: {
      en: 'Writes spike every few minutes. What should the DBA correlate next?',
      es: 'Las escrituras se disparan cada pocos minutos. Que debe correlacionar ahora el DBA?',
    },
    summary: {
      en: 'This looks like checkpoint-driven flushing, not Lazy Writer panic.',
      es: 'Esto parece flushing empujado por checkpoint, no panico de Lazy Writer.',
    },
    suspicion: {
    ...
    observations: [
      ...
      { label: { en: 'Pattern', es: 'Patron' }, value: 'Every 5 min', tone: 'amber' },
    ],
    choices: [
      { id: 'checkpoint', label: { en: 'Checkpoint burst', es: 'Rafaga de checkpoint' }, explanation: { en: 'Correct: the counters line up with checkpoint.', es: 'Correcto: los contadores encajan con checkpoint.' } },
      { id: 'memory', label: { en: 'Lazy Writer pressure', es: 'Presion de Lazy Writer' }, explanation: { en: 'Lazy Writer is quiet here.', es: 'Aqui Lazy Writer esta tranquilo.' } },
      { id: 'blocking', label: { en: 'Blocking by writers', es: 'Bloqueo por writers' }, explanation: { en: 'The periodic counter pattern points elsewhere first.', es: 'El patron periodico de contadores apunta primero a otra cosa.' } },
    ],
    correctChoiceId: 'checkpoint',
    diagnosis: {
      en: 'Review target recovery time and the write burst source before tuning memory.',
      es: 'Revisa target recovery time y la fuente de la rafaga antes de ajustar memoria.',
    },
    dbaFocus: { en: 'Tie the burst to recovery-driven flushing.', es: 'Ata la rafaga al flushing guiado por recovery.' },
    dbaActions: [
      ...
      { en: 'If ETL matches the burst, inspect that workload too.', es: 'Si ETL coincide con la rafaga, inspecciona tambien esa carga.' },
    ],
    caution: { en: 'Checkpoint and Lazy Writer write dirty pages for different reasons.', es: 'Checkpoint y Lazy Writer escriben paginas sucias por razones distintas.' },
    query: `SELECT counter_name, cntr_value
FROM sys.dm_os_performance_counters
...
WHERE database_id > 4;`,
    followUp: [
      { en: 'If one database owns the burst, start there.', es: 'Si una base domina la rafaga, empieza por ahi.' },
