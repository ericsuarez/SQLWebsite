...
import {
  TEMPDB_IO_BASELINES,
  TEMPDB_IO_DIAGNOSTIC_CASES,
} from '../../data/tempdbIoPlaybooks';
...
function baselineStateFor(value: number, healthy: number, warning: number) {
...
export function TempDBAndIO() {
  ...
  const activeBaseline = TEMPDB_IO_BASELINES.find((metric) => metric.id === activeBaselineId) ?? TEMPDB_IO_BASELINES[0];
  ...
  useEffect(() => {
    setBaselineProbeValue(activeBaseline.defaultValue);
  }, [activeBaseline]);
  ...
  const activeBaselineState = baselineStateFor(
    baselineProbeValue,
    ...
    <div className="flex h-full flex-col gap-6 text-slate-200">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="rounded-3xl border border-cyan-500/20 bg-cyan-500/10 p-4">
            <div className="text-[11px] font-bold uppercase tracking-[0.2em] text-cyan-300">
              {language === 'es' ? 'Regla rapida' : 'Quick rule'}
            </div>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.06fr)_380px]">
          <div className="grid gap-3 md:grid-cols-2">
            {filteredBaselines.map((metric) => {
              ...
              const metricStyle = BASELINE_STATE_STYLES[metricState];

              return (
                <button
                  key={metric.id}
                  onClick={() => setActiveBaselineId(metric.id)}
                  className={cn(
          ...
          <div className="space-y-4">
            <div className={cn('rounded-3xl border p-5', BASELINE_STATE_STYLES[activeBaselineState].panel)}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <span className={cn('rounded-full border px-3 py-1 text-xs font-bold', BASELINE_STATE_STYLES[activeBaselineState].chip)}>
                  {baselineProbeValue}
                  {activeBaseline.unit}
                </span>
              </div>
              ...
              <div className="mt-5">
                <div className="flex items-center justify-between gap-3 text-[11px] font-bold uppercase tracking-[0.18em] text-white/45">
                  <span>{language === 'es' ? 'Simula tu valor' : 'Simulate your value'}</span>
                  <span>
                    {activeBaseline.min}
                    {activeBaseline.unit} - {activeBaseline.max}
                    {activeBaseline.unit}
                  </span>
                </div>
                <input
                  type="range"
                  min={activeBaseline.min}
                  max={activeBaseline.max}
                  step={activeBaseline.step}
                  value={baselineProbeValue}
                  onChange={(event) => setBaselineProbeValue(Number(event.target.value))}
      ...
      <div className="glass-panel rounded-3xl border border-white/10 p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-4xl">
            <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-white/40">
              {language === 'es' ? 'Incidencias guiadas tipo DBA' : 'DBA-guided incidents'}
            </p>
            <h3 className="mt-2 text-2xl font-bold text-white">
              {language === 'es'
            ...
            <p className="mt-3 max-w-5xl text-sm leading-7 text-white/75">
              {language === 'es'
                ? 'La idea no es memorizar una query aislada. La idea es pensar como un DBA: sintoma, evidencia, confirmacion y siguiente paso seguro.'
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.08fr)_360px]">
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {activeDiagnostic.badges.map((badge) => (
                    <span key={`${activeDiagnostic.id}-${badge}`} className={cn('rounded-full border px-3 py-1 text-[11px] font-bold uppercase tracking-[0.16em]', diagnosticAccent.chip)}>
                      {badge}
                    </span>
                  ))}
                </div>
          ...
          <div className="space-y-4">
            <DBAActionBoard
              ...
              title={{
                en: 'What the DBA does now',
                es: 'Que hace ahora el DBA',
              }}
