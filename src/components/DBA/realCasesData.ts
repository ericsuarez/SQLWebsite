...
-- Hyper-V: "Hyper-V Hypervisor VP\\CPU Wait Time"`,
    ...
    steps: [
      ...
      { log: 'I/O spike hits! OS, Data, and Log are on a single SCSI controller. Queue depth maxes out at 64.', phase: 'wait', spids: [{ spid: 130, status: 'suspended', waitType: 'PAGEIOLATCH_SH', label: 'SCSI Queue Full' }, { spid: 131, status: 'suspended', waitType: 'WRITELOG', label: 'SCSI Queue Full' }], sqlos: { schedulers: 8, workers: 8, runnable: 0, suspended: 8, waitType: 'PAGEIOLATCH_SH', waitMs: 450 }, buffer: { totalPages: 1000, usedPages: 500, dirtyPages: 10, evictedPages: 0 }, highlight: 'io' },
      { log: 'Host runs out of RAM. Memory balloon driver inflates inside SQL VM, stealing RAM back to the host!', phase: 'error', spids: [{ spid: 132, status: 'evicted', waitType: 'RESOURCE_SEMAPHORE', label: 'Balloon inflating' }], sqlos: { schedulers: 8, workers: 8, runnable: 0, suspended: 8, waitType: 'RESOURCE_SEMAPHORE', waitMs: 900 }, buffer: { totalPages: 1000, usedPages: 500, dirtyPages: 10, evictedPages: 300 }, highlight: 'plan' },
      { log: 'Fix: Reserve vCPUs and full RAM. Disable ballooning. Separate OS, Data, Log to Paravirtual SCSI controllers. CPU Ready < 1%.', phase: 'done', spids: [{ spid: 130, status: 'running', label: 'Healthy VM ✓' }, { spid: 131, status: 'running', label: 'Multi-SCSI ✓' }], sqlos: { schedulers: 8, workers: 4, runnable: 4, suspended: 0 }, buffer: { totalPages: 1000, usedPages: 700, dirtyPages: 5, evictedPages: 0 } },
    ],
  },
  {
    id: 'missingindex',
