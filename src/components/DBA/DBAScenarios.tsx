...
import { Database, Zap, FileWarning, ShieldCheck, HardDrive, Cpu, Code2 } from 'lucide-react';
...
export function DBAScenarios() {
    const { t } = useLanguage();
    const [activeTab, setActiveTab] = useState<'pageSplit' | 'ifi' | 'creation'>('pageSplit');

    // Page Split State
    const [fillFactor, setFillFactor] = useState(100);
    const [pages, setPages] = useState<number[][]>([[1, 2, 3, 4, 5]]);
    const maxRows = 6; // To simulate visually
    ...
    const insertRow = () => {
        let newPages = [...pages];
        let lastPage = [...newPages[newPages.length - 1]];

        // Simulate fill factor limit logic visually:
        const targetMax = Math.ceil(maxRows * (fillFactor / 100));

        if (lastPage.length >= targetMax) {
            // Split required!
            const midPoint = Math.floor(lastPage.length / 2);
            const splitLeft = lastPage.slice(0, midPoint);
            const splitRight = lastPage.slice(midPoint);
            splitRight.push(splitRight.length + splitLeft.length + 1); // Add the new row

            newPages[newPages.length - 1] = splitLeft;
            newPages.push(splitRight);
        } else {
            // Just insert
            lastPage.push(lastPage.length + 1);
    ...
    // Calculate a rough fragmentation proxy: number of partially empty pages
    const fragmentation = Math.round(
        pages.length > 1
            ? (pages.filter(p => (p.length / maxRows) < 0.8).length / pages.length) * 100
            : 0
    );

    useEffect(() => {
        if (dbStatus === 'creating') {
            const step = ifiEnabled ? 100 : 5; // IFI = instant, No IFI = slow zeroing
    ...
    return (
        <div className="flex flex-col h-full gap-6">
            ...
            <div className="flex gap-4 border-b border-white/10 pb-4">
                <button
                    onClick={() => setActiveTab('pageSplit')}
                    className={cn("px-4 py-2 rounded-lg font-bold transition-all", activeTab === 'pageSplit' ? "bg-rose-500/20 text-rose-400 border border-rose-500/50" : "bg-white/5 text-muted-foreground hover:bg-white/10")}
                >
                    {t('tabPageSplit')}
                </button>
                <button
                    onClick={() => setActiveTab('ifi')}
                    className={cn("px-4 py-2 rounded-lg font-bold transition-all", activeTab === 'ifi' ? "bg-orange-500/20 text-orange-400 border border-orange-500/50" : "bg-white/5 text-muted-foreground hover:bg-white/10")}
                >
                    {t('tabIfi')}
                </button>
                <button
                    onClick={() => setActiveTab('creation')}
                    className={cn("px-4 py-2 rounded-lg font-bold transition-all", activeTab === 'creation' ? "bg-purple-500/20 text-purple-400 border border-purple-500/50" : "bg-white/5 text-muted-foreground hover:bg-white/10")}
                >
                    {t('tabCreation')}
                </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
                {activeTab === 'pageSplit' && (
                    <div className="flex flex-col gap-6 h-full">
                        <div className="glass-panel p-6 rounded-2xl border-rose-500/30">
                            <div className="flex justify-between items-start mb-6 flex-col lg:flex-row gap-4">
                                <div>
                                ...
                                <div className="flex items-center gap-4 bg-black/40 p-3 rounded-lg border border-white/10 flex-wrap">
                                    ...
                                    <div className="flex flex-col px-2">
                                        ...
                                        <span className={cn("text-xl font-black", fragmentation > 50 ? "text-rose-500" : fragmentation > 20 ? "text-amber-500" : "text-emerald-500")}>
                                            {fragmentation}%
                                        </span>
                                    </div>
                                    <div className="w-px h-8 bg-white/10 hidden md:block" />
                                    <button onClick={insertRow} className="px-4 py-2 bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 rounded font-bold transition-colors">
                                        {t('insertRowBtn')}
                                    </button>
                                    <button onClick={resetPages} className="px-3 py-2 bg-white/5 hover:bg-white/10 text-muted-foreground rounded transition-colors">
                                        {t('resetBtn')}
                                    </button>
                                </div>
                            </div>
                                ...
                                <AnimatePresence>
                                    {pages.map((page, i) => (
                                        <motion.div
                                            ...
                                            className={cn(
                                                "w-48 h-64 border-2 rounded-xl flex flex-col p-2 gap-1 shadow-lg bg-black/40 relative overflow-hidden",
                                        ...
                                        >
                                            ...
                                            <div className="text-xs font-bold text-center border-b border-white/10 pb-1 mb-1 text-muted-foreground">
                                                Page {i + 1}
                                            </div>
                                            ...
                                            <AnimatePresence>
                                                {page.map((row, j) => (
                                                    <motion.div
                                                        ...
                                                        className="bg-emerald-500/20 text-emerald-300 text-xs p-1.5 rounded border border-emerald-500/30 flex justify-between"
                                                    >
                                                        <span>{t('rowData')}</span>
                                                        <span className="opacity-50">#{(j + 1)}</span>
                                                    </motion.div>
                                                ))}
                                            </AnimatePresence>
                                            ...
                                            <div className="text-[10px] text-center text-muted-foreground/50 border-t border-white/10 pt-1 mt-1 flex justify-between px-1">
                ...
                {activeTab === 'ifi' && (
                    <div className="flex flex-col gap-6 h-full">
                        <div className="glass-panel p-6 rounded-2xl border-orange-500/30">
                            ...
                            <div className="bg-black/40 p-6 rounded-xl border border-white/10 flex flex-col gap-6">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <div className={cn("w-12 h-6 rounded-full p-1 cursor-pointer transition-colors shadow-inner", ifiEnabled ? "bg-emerald-500" : "bg-white/10")} onClick={() => setIfiEnabled(!ifiEnabled)}>
                                            <motion.div
                                    ...
                                    <button
                                        ...
                                        className="px-6 py-2 bg-orange-500/20 hover:bg-orange-500/30 border border-orange-500/50 text-orange-300 rounded-lg font-bold transition-all disabled:opacity-50"
                                    >
                                        {t('allocateBtn')}
                                    </button>
                                </div>
                                ...
                                {dbStatus !== 'idle' && (
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <div className="h-4 bg-white/5 rounded-full overflow-hidden border border-white/10 relative">
                                            <motion.div
                                                className={cn("h-full", ifiEnabled ? "bg-emerald-500 shadow-[0_0_10px_#10b981]" : "bg-orange-500")}
                                                initial={{ width: 0 }}
                                        ...
                                        <div className="flex justify-between items-center mt-2">
                                            {!ifiEnabled && dbStatus === 'creating' && (
                                                <p className="text-xs text-rose-400 flex-1">{t('noticeDelay')}</p>
                                            )}
                                            <div className="flex items-center gap-2 text-xs font-mono ml-auto bg-black/50 px-3 py-1.5 rounded text-muted-foreground border border-white/10">
                                                ...
                                                <span className={ifiEnabled ? "text-emerald-400 font-bold" : "text-orange-400"}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="bg-black/40 p-5 border border-white/10 rounded-xl space-y-4">
                                <h4 className="font-bold text-amber-400 flex items-center gap-2">
                                    <Zap className="w-4 h-4" /> {t('ldfTitle')}
                                </h4>
                                <ul className="list-disc pl-4 text-sm text-muted-foreground space-y-2">
                                    <li>{t('ldfDesc1')}</li>
                                    <li>{t('ldfDesc2')}</li>
                                    <li>{t('ldfDesc3')}</li>
                                    <li><strong className="text-rose-400">{t('ldfDesc4')}</strong></li>
                                </ul>
                            </div>
                        </div>
            ...
            <TSqlModal
                ...
                diagnosticScript={
                    activeTab === 'pageSplit' ? {
                        '2019': `SELECT dm.index_id, dm.avg_fragmentation_in_percent, dm.page_count\nFROM sys.dm_db_index_physical_stats (DB_ID(), NULL, NULL, NULL, 'LIMITED') dm\nWHERE avg_fragmentation_in_percent > 10.0;`,
                        '2022': `SELECT dm.index_id, dm.avg_fragmentation_in_percent, dm.page_count\nFROM sys.dm_db_index_physical_stats (DB_ID(), NULL, NULL, NULL, 'LIMITED') dm\nWHERE avg_fragmentation_in_percent > 10.0;`,
                        '2025': `SELECT dm.index_id, dm.avg_fragmentation_in_percent, dm.page_count\nFROM sys.dm_db_index_physical_stats (DB_ID(), NULL, NULL, NULL, 'LIMITED') dm\nWHERE avg_fragmentation_in_percent > 10.0;`
                    } : {
                        '2019': `SELECT servicename, instant_file_initialization_enabled\nFROM sys.dm_server_services;`,
                        '2022': `SELECT servicename, instant_file_initialization_enabled\nFROM sys.dm_server_services;`,
                        '2025': `SELECT servicename, instant_file_initialization_enabled\nFROM sys.dm_server_services;`
                    }
                }
                remediationTitle={activeTab === 'pageSplit' ? t('dbaRemediationSplitTitle') : t('dbaRemediationIfiTitle')}
                remediationScript={
                    activeTab === 'pageSplit' ? {
                        '2019': `-- Rebuild with custom fillfactor\nALTER INDEX ALL ON TableName REBUILD WITH (FILLFACTOR = 80, ONLINE = ON);`,
                        '2022': `-- Rebuild with custom fillfactor and resumable operation\nALTER INDEX ALL ON TableName REBUILD WITH (FILLFACTOR = 80, ONLINE = ON, RESUMABLE = ON);`,
                        '2025': `-- Rebuild with custom fillfactor and optimized resumable operation\nALTER INDEX ALL ON TableName REBUILD WITH (FILLFACTOR = 80, ONLINE = ON, RESUMABLE = ON);`
                    } : {
                        '2019': `-- Cannot be enabled purely via T-SQL.\n-- 1. Open Local Security Policy (secpol.msc).\n-- 2. Local Policies -> User Rights Assignment.\n-- 3. Add SQL Server Service Account to "Perform volume maintenance tasks".\n-- 4. Restart SQL Server Service.`,
                        '2022': `-- Cannot be enabled purely via T-SQL.\n-- 1. Open Local Security Policy (secpol.msc).\n-- 2. Local Policies -> User Rights Assignment.\n-- 3. Add SQL Server Service Account to "Perform volume maintenance tasks".\n-- 4. Restart SQL Server Service.`,
                        '2025': `-- Cannot be enabled purely via T-SQL.\n-- 1. Open Local Security Policy (secpol.msc).\n-- 2. Local Policies -> User Rights Assignment.\n-- 3. Add SQL Server Service Account to "Perform volume maintenance tasks".\n-- 4. Restart SQL Server Service.`
                    }
                }
            />
        </div>
